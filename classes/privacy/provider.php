...
namespace assignsubmission_gradeqr\privacy;
...
class provider implements metadataprovider, \mod_assign\privacy\assignsubmission_provider {
    ...
    public static function get_metadata(collection $collection) : collection {
        $detail = [
                    ...
                    'gradeqr' => 'privacy:metadata:textpurpose'
                  ];
        $collection->add_database_table('assignsubmission_gradeqr', $detail, 'privacy:metadata:tablepurpose');
    ...
    public static function export_submission_user_data(assign_plugin_request_data $exportdata) {
        ...
        $plugin = $assign->get_plugin_by_type('assignsubmission', 'gradeqr');
        $submission = $exportdata->get_pluginobject();
        $editortext = $plugin->get_editor_text('gradeqr', $submission->id);
        ...
        if (!empty($editortext)) {
            ...
            $currentpath[] = get_string('privacy:path', 'assignsubmission_gradeqr');
            writer::with_context($context)
                    ->export_area_files($currentpath, 'assignsubmission_gradeqr', 'submissions_gradeqr', $submission->id)
    ...
    public static function delete_submission_for_context(assign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, 'assignsubmission_gradeqr',
                ASSIGNSUBMISSION_GRADEQR_FILEAREA);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_gradeqr', ['assignment' => $requestdata->get_assign()->get_instance()->id]);
    }
    ...
    public static function delete_submission_for_userid(assign_plugin_request_data $deletedata) {
        ...
        // Delete related files.
        $fs = get_file_storage();
        $fs->delete_area_files($deletedata->get_context()->id, 'assignsubmission_gradeqr', ASSIGNSUBMISSION_GRADEQR_FILEAREA,
                $submissionid);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_gradeqr', ['assignment' => $deletedata->get_assign()->get_instance()->id,
